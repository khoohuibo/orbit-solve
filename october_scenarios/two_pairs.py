...
pair_2_start_date = AbsoluteDate(2026, 4, 1, 0, 0, 00.000, utc)
astro_pair_2_star_date = Time('2026-04-01T00:00:00.000000000', format='isot', scale='utc')
...
accurate_raan_sso_2 = raan_from_ltan(astro_pair_2_star_date, ltan=22.5 * u.hourangle).value - 360
#raan = math.radians(accurate_raan_sso_1)  # right ascension of ascending node (SSO)
raan = math.radians(344.920)
raan_2 = math.radians(accurate_raan_sso_2)
lv = math.radians(0.0)    # True anomaly

bandwagon_i = math.radians(45.1)
bandwagon_raan = math.radians(0)
...
pair_2_propagator, _ = set_up_prop(i, omega, raan_2, lv, pair_2_start_date, inertialFrame, ITRF, rp=rp, ra=ra, max_drag=False)
...
derived2Orbit_alt, seed2Orbit, newEpoch, t_delta = get_ecef(date, absolutedate_to_datetime(pair_2_start_date), vel_abs, pos, vel, pv_list, inertialFrame, ITRF, inclination=i)

sma_2 = derived2Orbit_alt.getA()
inc_2 = derived2Orbit_alt.getI()
ecc_2 = derived2Orbit_alt.getE()
raan_alt_2= derived2Orbit_alt.getRightAscensionOfAscendingNode()
aop = derivedOrbit_alt.getPerigeeArgument()
lv_2 = seed2Orbit.getMeanAnomaly() + t_delta * mean_motion(satellite_mass, 600)

pair_2_1, _ = set_up_prop(i, omega, raan_2, lv, pair_2_start_date, inertialFrame, ITRF,rp=rp, ra=ra, max_drag=False)

pair_2_2, _ = set_up_prop(inc_2, aop, raan_alt_2, lv_2, newEpoch, inertialFrame, ITRF,rp=rp, ra=ra, max_drag=False, initialOrbit=derived2Orbit_alt)
...
while (extrapDate.compareTo(finalDate) <= 0.0):  
    ...
    if extrapDate.compareTo(pair_2_start_date) >= 0.0:
        ...
        #print(dist_to_pair_2)
        #print(pair_2_orbit.getA())
        ...
        #C_D_list.append(Vector3D.distance(derived_2_state.getPosition(), seed_2_state.getPosition())/1000)
        #pair_2_date.append(absolutedate_to_datetime(extrapDate))
    else:
...
with open('output/pair_2_both_sso_oct.csv', 'w') as f:
    writer = csv.writer(f)
...
axs[2,2].set_title("seed_2_A-C_list")
axs[3,0].plot(pair_2_date, B_C_list)
axs[3,0].set_title("pair_2_B-C_list")
axs[3,1].plot(pair_2_date, A_D_list)
axs[3,1].set_title("derived_2_A-D_list")
axs[3,2].plot(pair_2_date, C_D_list)
