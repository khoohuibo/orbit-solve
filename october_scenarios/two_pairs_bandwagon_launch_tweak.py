...
pair_2_start_date = AbsoluteDate(2026, 5, 1, 0, 0, 00.000, utc)
...
raan = math.radians(344.920)  # right ascension of ascending node (SSO)
lv = math.radians(0.0)    # True anomaly

bandwagon_i = math.radians(45.1) # inclination (Bandwagon)
bandwagon_raan = math.radians(0.0) # RAAN (Bandwagon)
#bandwagon_raan = math.radians(180)
bandwagon_lv = math.radians(0.0)
...
pair_2_propagator, _ = set_up_prop(bandwagon_i, aop, bandwagon_raan, bandwagon_lv, pair_2_start_date, inertialFrame, ITRF, rp=rp, ra=ra)
...
derived2Orbit_alt, seed2Orbit, newEpoch, t_delta = get_ecef(date, absolutedate_to_datetime(pair_2_start_date), vel_abs, pos, vel, pv_list, inertialFrame, ITRF, inclination=bandwagon_i)
...
pair_2_1, _ = set_up_prop(bandwagon_i, omega, bandwagon_raan, bandwagon_lv, pair_2_start_date, inertialFrame, ITRF,rp=rp, ra=ra, max_drag=False)

pair_2_2, _ = set_up_prop(bandwagon_i, aop, raan_2, lv_2, newEpoch, inertialFrame, ITRF,rp=rp, ra=ra, max_drag=False)
...
date = []
dist_list = []
seed_2_seed_dist_list = []
derived_2_seed_dist_list = []
derived_2_seed_2_dist_list = []
pair_2_date = []
...
pair_2_sma_list = []
derived_2_derived_dist_list = []
seed_2_derived_dist_list = []
...
while (extrapDate.compareTo(finalDate) <= 0.0):  
    ...
    if extrapDate.compareTo(pair_2_start_date) >= 0.0:
        ...
        seed_2_seed_dist_list.append(Vector3D.distance(seed_2_state.getPosition(), pv_state.getPosition())/1000)
        ...
        derived_2_seed_dist_list.append(Vector3D.distance(derived_2_state.getPosition(), pv_state.getPosition())/1000)

        derived_2_seed_2_dist_list.append(Vector3D.distance(derived_2_state.getPosition(), seed_2_state.getPosition())/1000)
        derived_2_derived_dist_list.append(Vector3D.distance(derived_2_state.getPosition(), seed_2_state.getPosition())/1000)
        seed_2_derived_dist_list.append(Vector3D.distance(derived_2_state.getPosition(), seed_2_state.getPosition())/1000)

    else:
        seed_2_seed_dist_list.append(0) # C - A
        seed_2_derived_dist_list.append(0) # C - B
        derived_2_derived_dist_list.append(0) # D - B
        derived_2_seed_dist_list.append(0) # D -A
        derived_2_seed_2_dist_list.append(0) # C-D
...
header = ['date', 'distance(Seed-Derived)', 'distance(Seed-Seed2)', 'distance(Seed-Derived2)', 'distance(Derived-Seed2)', 'distance(Derived-Derived2)', 'distance(Seed2-Derived2)', 'beta_angle_sun', 'derived_beta_angle_sun', 'sma', 'inc', 'ecc', 'raan', 'aop', 'lv']
with open('output/pair_2_bandwagon_launch_tweak_oct.csv', 'w') as f:
    writer = csv.writer(f)

    writer.writerow(header)
    for j in range(len(date)):
        writer.writerow([date[j], dist_list[j], seed_2_seed_dist_list[j], derived_2_seed_dist_list[j], seed_2_derived_dist_list[j], derived_2_derived_dist_list[j], derived_2_seed_2_dist_list[j], beta_list_sun[j], derived_beta_list_sun[j], sma_list[j], i_list[j], ecc_list[j], raan_list[j], aop_list[j], lv_list[j]])
...
axs[2,2].plot(pair_2_date, seed_2_seed_dist_list)
axs[2,2].set_title("seed_2_seed_dist_list")
axs[3,0].plot(pair_2_date, seed_2_derived_dist_list)
axs[3,0].set_title("pair_2_sma_list")
axs[3,1].plot(pair_2_date, derived_2_seed_dist_list)
axs[3,1].set_title("derived_2_seed_dist_list")
axs[3,2].plot(pair_2_date, derived_2_seed_2_dist_list)
axs[3,2].set_title("derived2 - seed2")
plt.show()