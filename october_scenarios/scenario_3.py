...
raan_new = raan_alt 
derivedpropagator, _ = set_up_prop(inc, aop, raan_new, lv_new, newEpoch, inertialFrame, ITRF, rp=rp, ra=ra, max_drag=True)
...
while (extrapDate.compareTo(finalDate) <= 0.0):  
    ...
    derived_sma_list.append(der_orbit.getA())
...
header = ['date', 'distance', 'beta_angle_sun', 'derived_beta_angle_sun', 'sma', 'der_sma', 'inc', 'ecc', 'raan', 'aop', 'lv']
with open('output/scenario_3_oct.csv', 'w') as f:
    writer = csv.writer(f)

    writer.writerow(header)
    for j in range(len(date)):
        writer.writerow([date[j], dist_list[j], beta_list_sun[j], derived_beta_list_sun[j], sma_list[j], derived_sma_list[j], i_list[j], ecc_list[j], raan_list[j], aop_list[j], lv_list[j]])
